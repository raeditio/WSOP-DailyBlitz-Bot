...
SECRET_KEY = b'iP-Fj1Htt3VS0VBErsU7dp07cbajF9H_Ne09JscBRh4='

class ONNXYOLO:
    def __init__(self, model_bytes, imgsz=640):
        ...
        # Extract names dictionary from ONNX metadata
    ...
    def letterbox(self, im, new_shape=(640, 640), color=(114, 114, 114), auto=False, stride=32):
        ...
        if auto:  # minimum rectangle padding (perfect for dynamic=True)
    ...
    def __call__(self, imgs, conf=0.25, iou=0.4, imgsz=None, verbose=False):
        if not isinstance(imgs, list):
            imgs = [imgs]
            
        target_size = imgsz or self.imgsz
        all_results = []
        
        # Helper classes to match the expected Ultralytics syntax
        class DummyBox:
            def __init__(self, xyxy, conf, cls_id):
                self.xyxy, self.conf, self.cls = [xyxy], [conf], [cls_id]
        ...
        for img in imgs:
            ...
            im_padded, r, (dw, dh) = self.letterbox(img, new_shape=target_size, auto=True)
            
            input_img = cv2.cvtColor(im_padded, cv2.COLOR_BGR2RGB)
            ...
            # 3. Parse NMS-Free End-to-End Outputs [x1, y1, x2, y2, score, class_id]
            predictions = outputs[0][0] 
            
            final_boxes = []
            
            # Handle edge case where there is only 1 prediction (1D array instead of 2D)
            ...
            for pred in predictions:
                if len(pred) >= 6:
                    score = float(pred[4])
                    if score > conf:
                        # Scale coordinates back to original image size (undo the letterbox padding)
                        x1 = (pred[0] - dw) / r
                        y1 = (pred[1] - dh) / r
                        x2 = (pred[2] - dw) / r
                        y2 = (pred[3] - dh) / r
                        ...
                        final_boxes.append(DummyBox([x1, y1, x2, y2], score, class_id))
                        
            all_results.append(DummyResult(final_boxes))
            
        return all_results

def load_encrypted_model(enc_path, imgsz=640):
    ...
    return ONNXYOLO(decrypted_data, imgsz=imgsz)
...
def scan_board_cascaded(img_rgb, zone_model, card_model, conf_zone=0.25, conf_card=0.10, zone_imgsz=640, card_imgsz=1024, debug=False):
    ...
    for result in zone_results:
        for box in result.boxes:
            ...
            if label in ['community_card', 'left_hand', 'right_hand']:
                if label not in detected_zones:
    ...
    # --- FIX 3: Spatial Deduplication Guardrail ---
...
def run_bot_loop(api):
    ...
    with mss.MSS() as sct:
        ...
        while not api.exit_flag:
            ...
            # Reverted conf_zone to 0.25 as confidence was not the root issue
            ...
            if matches and len(matches) == 9:
                ...
                if success:
                    print("\nHand evaluated & clicked! Waiting for the next round...")
                time.sleep(0.5)
            else:
